...
5. `20260723000004_employees_slack_office_columns.sql` — **employees に slack_user_id / office_location 列を追加（切替前に必須）**
6. `20260723000005_action_plans_and_leave_balances.sql` — pulse のアクションプラン表 / attendance の有給付与日数マスタ（任意・入れると当該箇所が実データ化）

> ⚠️ 2〜5 はいずれも「要レビュー」。適用前に SQL の可視範囲（誰が何を見てよいか）を確認すること。特に給与・評価・健康・在留。
> ⚠️ **5 は `NEXT_PUBLIC_DEMO_MODE=false` にする前に必ず適用**すること。実データ経路の employees クエリが slack_user_id / office_location 列を SELECT するため、未適用のままフラグを false にすると全ページで社員取得が失敗する（列追加は additive で安全）。これで slack-handlers の名簿同期（現状 silently 全滅）も復旧する。
...
- ~~`slack_user_id` 列が employees に無い~~ → **対応済み**（migration 20260723000004。適用で Slack 名簿同期・リマインド復旧）
- ~~action_plans テーブルが無い~~ → **対応済み**（migration 20260723000005。pulse のアクションが実データ化）
- ~~有給付与日数マスタが無い~~ → **対応済み**（migration 20260723000005 の leave_balances。attendance の有給取得率が実データ化）
- 月次トレンドの集計テーブルが無い → pulse の推移グラフは実データ源が無く空（現状は既定系列にフォールバック）。月次スナップショット表＋集計ジョブが要る（未対応）
- 休暇の承認/却下 API が無い → ボタンは toast のみ（未永続化。hr_admin 認可付き更新エンドポイントが要る）
