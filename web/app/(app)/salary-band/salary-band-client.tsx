...
export function SalaryBandClient({
  bands,
  avgAnnualLabel,
  nextCycleLabel,
}: {
  bands: BandRow[];
  avgAnnualLabel: string;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <KpiTile label="平均年収" value={avgAnnualLabel} unit="" tone="success" icon={Wallet} />
        <KpiTile label="次回昇給サイクル" value={nextCycleLabel} unit="" tone="muted" icon={TrendingUp} />
      </div>
      ...
      <Card>
        <CardContent className="p-4">
          ...
          {bands.length === 0 ? (
            <p className="py-8 text-center text-xs text-muted-foreground">給与帯データがありません</p>
          ) : (
