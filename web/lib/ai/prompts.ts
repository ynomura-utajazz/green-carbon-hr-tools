...
export type OneOnOneExtractInput = {
  notes: string;
  member_name: string;
  manager_name: string;
  meeting_date: string;
...
export type ExtractedOneOnOneAction = {
  title: string;
  assignee: "self" | "manager" | "other";  // self = member, manager = the manager
  assignee_name?: string;                  // assignee="other" 時に名前を入れる
  due_date: string | null;                 // ISO YYYY-MM-DD
};

export type OneOnOneExtractOutput = {
  topics: string[];           // 標準タグの中から最大 5 個
  mood: "great" | "good" | "ok" | "down" | "bad" | null;
  actions: ExtractedOneOnOneAction[];
  summary_oneliner: string;
};

export function buildOneOnOneExtractPrompt(i: OneOnOneExtractInput): {
  system: string; user: string;
} {
  const standardTopics = ONEONONE_TOPIC_TAGS.join(" / ");

  const system = `あなたは 1on1 ミーティングの議事メモを解析する HR AI です。
日本語の自由記述メモから、(1) トピックタグ、(2) 全体の気分、(3) アクション項目、(4) 1 行サマリを抽出します。

## 出力フォーマット（純粋な JSON のみ。コードフェンス / マークダウン禁止）
{
  "topics": ["タグ1", "タグ2", ...],
  "mood": "great" | "good" | "ok" | "down" | "bad" | null,
  "actions": [
    {
      "title": "やること（動詞始まり、簡潔に 1 行）",
      "assignee": "self" | "manager" | "other",
      "assignee_name": "Other の場合のみ名前",
      "due_date": "YYYY-MM-DD or null"
    }
  ],
  "summary_oneliner": "ミーティングの要旨を 1 行で（30 字以内）"
}

## トピックタグの選び方
必ず以下の標準タグの中から、内容に合致するものを最大 5 個選んでください:
${standardTopics}

該当なしの場合は「その他」を選ぶ。新規タグは作らない。

## mood の推定
- メモ全体のトーン・前向きさから推定
- ポジティブ・明るい → good / great
- 中立・淡々 → ok
- 不安・疲弊・愚痴 → down / bad
- 判断不可なら null

## actions の抽出
- 「〜します」「〜お願い」「〜やる」「次回までに〜」等の動作要素を抽出
- assignee: "self" = メンバー本人, "manager" = マネージャー, "other" = それ以外
- due_date: 「来週」→ meeting_date + 7 日, 「今月末」→ 月末日, 不明 → null
- 雑談・感想は無視`;

  const user = `## 会議情報
日付: ${i.meeting_date}
メンバー（self）: ${i.member_name}
マネージャー: ${i.manager_name}

## 議事メモ
${i.notes}

JSON でトピック・気分・アクション・サマリを抽出してください。`;
